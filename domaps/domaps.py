...
class SpatialDataSet:
    ...
    acquisition_set_dict = {
        ...
        "SILAC - MQ": [
            "[Rr]atio.[Hh]/[Ll](?!.[Vv]aria|.[Cc]ount)",
            "[Rr]atio.[Hh]/[Ll].[Vv]ariability.\[%\]",
            "[Rr]atio.[Hh]/[Ll].[cC]ount",
        ],
    ...
    Spectronaut_columnRenaming = {
        ...
        "PG.Qvalue": "Q-value",
        "PG.Cscore": "C-Score",
        "PG.ProteinGroups": "Original Protein IDs",
        "PG.RunEvidenceCount": "MS/MS count",
        "PG.Quantity": "LFQ intensity",
    }
        ...
        **kwargs,
        ...
        if legacy == True:
            ...
            self.markerproteins = {
                k: v.replace(" ", "").split(",")
                for k, v in zip(
                    marker_table["Cluster"], marker_table["Members - Protein IDs"]
                )
            }
        ...
        if legacy:
            ...
            if settings["source"] == "MaxQuant":
                if settings["acquisition"] == "LFQ":
                    ...
                    raise ValueError(
                        f"No legacy configuration for {settings['source']} and {settings['acquisition']}."
                    )
            ...
            ds = SpatialDataSet(
                ...
                reannotation_source=parse_reannotation_source(
                    settings["reannotate"], settings["reannotation_source"]
                ),
            ...
            settings["reannotation_source"] = parse_reannotation_source(
                settings["reannotate"], settings["reannotation_source"]
            )
    ...
    def process_input(self):
        ...
        if self.orientation == "long":
            df_index = format_data_long(
                ...
                index_cols=self.kwargs["columns_annotation"]
        ...
        if self.reannotate_genes:
            ...
            genes = [
                ...
                for g, p in zip(
                    df_index.index.get_level_values("Gene names"),
                    df_index.index.get_level_values("Protein IDs"),
                )
            ]
            df_index.reset_index("Gene names", inplace=True, drop=True)
            df_index.set_index(
                pd.Index(name="Gene names", data=genes), inplace=True, append=True
            )
        ...
        df_index = df_index.join(
            ...
            how="left",
            on="Protein IDs",
        ).set_index("Compartment", append=True)
        df_index.columns = pd.MultiIndex.from_tuples(
            df_index.columns, names=["Set", "Map", "Fraction"]
        )
        ...
        if len(self.kwargs["quality_filter"]) == 0:
            ...
            [
                el in ["SILAC_countvar", "msms_count", "consecutive"]
                for el in self.kwargs["quality_filter"]
            ]
        ):
            ...
            if "consecutive" in self.kwargs["quality_filter"]:
                df_filtered = filter_consecutive(
                    df_filtered, self.consecutive, sets=dict(abundance=self.mainset)
                )
        ...
        if self.kwargs["input_samplenormalization"]:
            df_transformed = normalize_samples(
                df_transformed, method=self.kwargs["input_samplenormalization"]
            )
            ...
            processing_steps.append(
                f"Normalizing samples by {self.kwargs['input_samplenormalization']}"
            )
        ...
        if self.acquisition == "SILAC":
            ...
            df_transformed = filter_consecutive(
                df_transformed,
                len(self.fractions),
                sets=dict(abundance=self.transformed_mainset),
            )
            processing_steps.append("Filtering for complete ratio profiles")
        if self.acquisition == "LFQ":
            df_transformed = (
                df_transformed.stack("Map").replace(np.NaN, 0).unstack("Map")
            )

        ## Normalize profiles
        df_01_stacked = normalize_sum1(
            df_transformed, sets={"abundance": self.transformed_mainset}
        ).stack("Fraction")
        ...
        df_log_stacked = df_log_stacked.drop(self.transformed_mainset, axis=1).join(
            ...
            .replace(0, np.nan)
            .transform(np.log2)
            .rename({self.transformed_mainset: "log profile"}, axis=1)
        )
        ...
        df_01_comparison = df_01_comparison.reset_index(
            level=["Gene names", "Original Protein IDs", "Protein IDs", "Compartment"]
    ...
    def plot_log_data(self):
        ...
        log_histogram = px.histogram(
            self.df_log_stacked.reset_index().sort_values(
                ["Map", "Fraction"], key=natsort_list_keys
            ),
            x="log profile",
            facet_col="Fraction",
            facet_row="Map",
            ...
            labels={
                "log profile": "log tranformed data ({})".format(
    ...
    def quantity_profiles_proteinGroups(self):
        ...
        try:
            ...
            df_01_intersection = df_01_MapStacked.groupby(level="Protein IDs").filter(
                lambda x: len(x) == len(self.map_names)
            )
        ...
        for df_intersection in [df_index_intersection, df_01_intersection]:
            for fraction in self.fractions:
                ...
                npgF_f = (
                    df_intersection_frac.unstack("Map")
                    .isnull()
                    .sum(axis=1)
                    .value_counts()
                )
        ...
        df_npg.sort_values(
            ["Fraction", "Protein Groups present in:"],
            inplace=True,
    ...
    def perform_pca(self, n=3):
        ...
        if "SILAC" in self.acquisition and self.legacy:
            df_01orlog_fracunstacked = (
                self.df_log_stacked["log profile"].unstack("Fraction").dropna()
            )
            df_01orlog_MapFracUnstacked = (
                self.df_log_stacked["log profile"].unstack(["Fraction", "Map"]).dropna()
            )

        else:
            df_01orlog_fracunstacked = (
                self.df_01_stacked["normalized profile"].unstack("Fraction").dropna()
            )
            df_01orlog_MapFracUnstacked = (
            ...
            for marker in markerproteins[clusters]:
                try:
                    plot_try_pca = df_pca_filtered.xs(
                        marker, level="Protein IDs", drop_level=False
    ...
    def calc_biological_precision(self):
        ...
        if len(df_alldistances_individual_mapfracunstacked) == 0:
            self.df_distance_noindex = pd.DataFrame(
                columns=["Protein IDs", "Gene names", "Map", "Cluster", "distance"]
            )
            ...
            self.df_distance_noindex = (
                df_alldistances_aggregated_mapunstacked.stack("Map")
                .reset_index()
            ...
            self.df_allclusters_clusterdist_fracunstacked_unfiltered = (
                df_allclusters_01_unfiltered_mapfracunstacked.stack("Map")
            )
            # same as before, but now already abs
            self.df_allclusters_clusterdist_fracunstacked = (
                df_alldistances_individual_mapfracunstacked.stack("Map")
            )
    ...
    def get_marker_proteins_unfiltered(self, cluster):
        ...
        df_cluster_unfiltered.set_index(
            pd.Index(np.repeat(cluster, len(df_cluster_unfiltered)), name="Cluster"),
            append=True,
            inplace=True,
        )
        ...
        for m in maps:
            if distance_measure == "manhattan":
                d_m = pw.manhattan_distances(
                    df_cluster.xs(m, level="Map", axis=1),
                    ref_profile.xs(m, level="Map", axis=1),
                )
            else:
    ...
    def profiles_plot(self, map_of_interest="Map1", cluster_of_interest="Proteasome"):
        ...
        try:
            df_setofproteins = self.df_allclusters_01_unfiltered_mapfracunstacked.xs(
                cluster_of_interest, level="Cluster", axis=0
            )
            df_setofproteins_median = (
                df_setofproteins.dropna()
                .xs(map_of_interest, level="Map", axis=1)
                .median(axis=0)
            )

            # fractions get sorted
            df_setofproteins = df_setofproteins.xs(
                map_of_interest, level="Map", axis=1
            ).stack("Fraction")
            ...
            abundance_profiles_and_median_figure = (
                abundance_profiles_figure.add_scatter(
                    x=df_setofproteins_median["Fraction"],
    ...
    def distance_boxplot(self, cluster_of_interest="Proteasome"):
        ...
        try:
            ...
            self.proteins_quantified_across_all_maps = (
                df_cluster_xmaps_distance_with_index.unstack("Map").shape[0]
            )
            ...
            distance_boxplot_figure.update_layout(
                ...
                xaxis=go.layout.XAxis(
                    linecolor="black", linewidth=1, title="Map", mirror=True
                ),
    ...
    def calculate_pairwise_correlations(self, metric="cosine correlation"):
        ...
        df = (
            self.df_01_stacked.unstack("Map")
            .xs("normalized profile", axis=1, level="Set")
            .copy()
        )
            ...
            for i, mapi in enumerate(maps):
                for j, mapj in enumerate(maps):
                    ...
                    dist = pw.paired_distances(
                        df_m.xs(mapi, level="Map", axis=0).values,
            ...
            try:
                for maps in map_names:
                    df_dist_map_cluster = df_distance_map_cluster_gene_in_index.xs(
                        (clusters, maps), level=["Cluster", "Map"], drop_level=False
                    )
                    statistic_table = {
    ...
    def reframe_df_01ORlog_for_Perseus(self, df_01ORlog):
        ...
        index_ExpMap = (
            df_01ORlog_svm.index.get_level_values("Map")
            + "_"
            + df_01ORlog_svm.index.get_level_values("Fraction")
        )
        ...
        df_01ORlog_svm.rename(
            index={"undefined": np.nan}, level="Compartment", inplace=True
        )
...
class SpatialDataSetComparison:
    ...
    def read_jsonFile(self):  # , content=None
        ...
        for exp_name in json_dict.keys():
            for data_type in json_dict[exp_name].keys():
                if data_type == "0/1 normalized data":
                    ...
                    df_01_toadd.insert(
                        0, "Experiment", np.repeat(exp_name, len(df_01_toadd))
                    )
        ...
        df_01_combined = df_01_combined.set_index(
            ["Experiment", "Gene names", "Compartment"]
        )
        if (
            "Original Protein IDs" in df_01_combined.columns
            and "Protein IDs" in df_01_combined.columns
        ):
            df_01_combined.set_index(
                ["Original Protein IDs"], append=True, inplace=True
            )
            ...
            df_01_combined.set_index(
                pd.Index(df_01_combined["Protein IDs"], name="Original Protein IDs"),
                append=True,
                inplace=True,
            )
            df_01_combined.drop("Protein IDs", axis=1, inplace=True)
        ...
        if len(set(df_01_combined.index.get_level_values("Experiment"))) != len(
            self.exp_names
        ):
            raise ValueError(
        ...
        index_ExpMap = (
            df_01_filtered_combined.index.get_level_values("Experiment")
            + "_"
            + df_01_filtered_combined.index.get_level_values("Map")
        )
        ...
        columns = (
            df_01_filtered_combined.unstack(["Experiment"])
        ...
        compartments = sorted(
            set(df_01_filtered_combined.index.get_level_values("Compartment"))
        )
    ...
    def perform_pca_comparison(self, n=3):
        ...
        for exp in self.exp_names:
            df_exp = (
                self.df_01_filtered_combined.stack("Fraction")
                .unstack(["Experiment", "Map", "Exp_Map"])[exp]
                .mean(axis=1)
                .to_frame(name=exp)
            )
            df_mean = pd.concat([df_mean, df_exp], axis=1)
        df_mean = (
            df_mean.rename_axis("Experiment", axis="columns")
            .stack("Experiment")
            .unstack("Fraction")
        )
        ...
        df_cluster_pca = (
            ...
            .merge(df_cluster, how="right", on="Protein IDs")
            .dropna()
        )
        ...
        try:
            ...
            df_setofproteins_PCA = df_setofproteins_PCA.assign(
                Experiment_lexicographic_sort=pd.Categorical(
                    df_setofproteins_PCA["Experiment"],
    ...
    def get_marker_proteins(self, experiments, cluster):
        ...
        if any(
            [
                el not in df_cluster.columns.get_level_values("Experiment")
        ...
        drop_experiments = [
            el
            for el in df_cluster.columns.get_level_values("Experiment")
        ...
        df_cluster.set_index(
            pd.Index(np.repeat(cluster, len(df_cluster)), name="Cluster"),
            append=True,
            inplace=True,
        )
    ...
    def calc_cluster_distances(
        ...
        df_distances.columns = pd.MultiIndex.from_tuples(
            df_distances.columns, names=["Experiment", "Map"]
        )
        return df_distances
    ...
    def calc_biological_precision(self, experiments=None, clusters=None):
        ...
        df_distances = (
            df_distances.stack(["Experiment", "Map"])
            .reset_index()
            .sort_values(["Experiment", "Gene names"])
            .rename({0: "distance"}, axis=1)
        )
        df_distances.insert(
            0,
            ...
            [
                "_".join([e, m])
                for e, m in zip(df_distances["Experiment"], df_distances["Map"])
            ],
        )
    ...
    def plot_intramap_scatter_cluster(
        ...
        df_distance_comp["Experiment_lexicographic_sort"] = pd.Categorical(
            df_distance_comp["Experiment"], categories=multi_choice, ordered=True
        )
        df_distance_comp.sort_values(
            ["Experiment_lexicographic_sort", "Map"], inplace=True
        )

        if collapse_maps == False:
            # get only values form experiment of interest
            df_distance_selectedExp = df_distance_comp.loc[
                df_distance_comp["Experiment"].isin(multi_choice)
            ]
            # get only values form cluster of interest
            df_distance_selectedExp = df_distance_selectedExp.loc[
                df_distance_selectedExp["Cluster"] == cluster_of_interest_comparison
            ]
            ...
            if df_distance_selectedExp.shape[0] == 0:
                ...
                for i, exp in enumerate(multi_choice):
                    df_plot = df_distance_selectedExp[
                        df_distance_selectedExp["Experiment"] == exp
                    ]
                ...
                individual_distance_boxplot_figure.update_layout(
                    ...
                    height=500,
                    xaxis=go.layout.XAxis(
                        linecolor="black", linewidth=1, title="Experiment", mirror=True
                    ),
                    yaxis=go.layout.YAxis(
                        linecolor="black", linewidth=1, title="Distance", mirror=True
                    ),
            ...
            df_distance_selectedExp_global.set_index(
                ["Gene names", level_of_interest, "Cluster"], inplace=True
            )
    ...
    def plot_intramap_scatter(
        ...
        df = df.groupby(["Cluster", "Experiment", "Map"]).filter(
        ...
        if aggregate_maps is not False:
            ...
            df = pd.DataFrame(
                df.unstack("Map").apply(aggregate_maps, axis=1), columns=["distance"]
            )
        ...
        if aggregate_proteins is not False:
            ...
            df = pd.DataFrame(
                df.unstack(["Protein IDs", "Gene names"]).apply(
                    aggregate_proteins, axis=1
                ),
                columns=["distance"],
            )
        ...
        df = df.sort_values("distance", ascending=False).sort_index(
            ...
            key=lambda x: pd.Index(
                [multi_choice.index(el) for el in x], name="Experiment"
            ),
        )
        ...
        if plot_type == "strip":
            ...
            plot = px.bar(
                df.reset_index(), color="Cluster", barmode="stack", **plotargs
            )
            plot.update_layout(
        ...
        if (
            ...
            and highlight in df.index.get_level_values("Cluster")
    ...
    def quantity_pr_pg_barplot_comparison(self, multi_choice=["Exp1", "Exp2"]):
        ...
        df_quantity_pr_pg_combined = df_quantity_pr_pg_combined[
            df_quantity_pr_pg_combined["Experiment"].isin(multi_choice)
        ]
    ...
    def venn_sections(self, multi_choice_venn=["Exp1"], omit_size=300):
        ...
        if "Sequence" not in self.df_01_filtered_combined.index.names:
            ...
            sets_proteins_intersection = [
                set(
                    self.df_01_filtered_combined.xs(i, axis=0, level="Experiment")
                    .unstack(["Map", "Exp_Map"])
                    .dropna()
                    .index.get_level_values("Protein IDs")
                )
    ...
    def calculate_global_scatter(
        ...
        for exp in self.exp_names:
            df_m = (
                df.xs(exp, level="Experiment", axis=0)
                .unstack("Map")
                .dropna(axis=1, how="all")
                .dropna()
                .stack("Map")
            ...
            if " to " in metric:
                ...
                for m in maps:
                    dist_m = pd.DataFrame(
                        metrics[metric][1](
                            df_m.xs(m, level="Map", axis=1), df_profiles
                        ),
                        columns=[m],
                    )
    ...
    def plot_overview(self, multi_choice, clusters, quantile):
        ...
        depth = (
            ...
            .query("Experiment in @multi_choice")[
                ["Experiment", "number of protein groups"]
            ]
    ...
    def plot_svm_summary(
        ...
        df = df.sort_values(
            "Experiment", key=lambda x: [multi_choice.index(el) for el in x]
        )
...
class SVMComp:
 ...
 Class assignments:   {}""".format(
    ...
    def _get_markers(self, which="shared"):
        ...
        if which == "shared":
            shared = (
                self.df.unstack(["Experiment", "Map"])
                .dropna()
            ...
            unshared = (
                self.df.unstack(["Experiment", "Map"])
                .drop(
                    self.df_01_filtered_combined.unstack(["Experiment", "Map"])
                    .dropna()
                    .index,
                    axis=0,
                )
    ...
    def _grid_search(
        self,
        train=pd.DataFrame(columns=["Protein IDs", "Compartment"]),
        ...
        for exp in self.experiments:
            ...
            data = data.reset_index(level=["Compartment"], drop=True).loc[
                train["Protein IDs"], :
            ]
    ...
    def _best_estimator(
        accuracies=pd.DataFrame(
            columns=pd.MultiIndex.from_tuples(
                [(".*", "accuracy")], names=["Experiment", "Value"]
            ),
    ...
    def _predict_svm(
        ...
        for exp in self.experiments:
            ...
            data_all = (
                self.df.xs(exp, level="Experiment", axis=0).unstack("Map").dropna()
            )
            data = data_all.reset_index(level=["Compartment"], drop=True).loc[
                train["Protein IDs"], :
            ]
...
def reframe_df_01_fromJson_for_Perseus(json_dict):
            ...
            if (
                ...
                df_01_combined.drop(
                    [
                        ...
                        if not col.startswith("normalized profile")
                    ]
                )
                df_01_combined.columns = pd.MultiIndex.from_tuples(
                ...
                df_01_combined.rename(
                    columns={"normalized profile": exp_name}, inplace=True
                )
    ...
    index_ExpMap = (
        df.index.get_level_values("Experiment") + "_" + df.index.get_level_values("Map")
    )
    index_ExpMap.name = "Exp_Map"
...
def relabel_groups(exp, pgs, genes, sep=";"):
    ...
    return pd.DataFrame(
        out, columns=["Experiment", "Protein IDs", "Gene names", "merge type"]
    )
...
def align_datasets(df):
    ...
    df_01.set_index(
        pd.Index(
            [
                split_ids(el)
                for el in df_01.index.get_level_values("Original Protein IDs")
            ],
            name="Protein IDs",
        ),
        append=True,
    ...
    new_labels = relabel_groups(
        compl.index.get_level_values("Experiment"),
    ...
    index_mapping.set_index(
        ["Experiment", "Gene names", "Protein IDs", "merge type"], inplace=True
    )
    ...
    df_01_aligned.index = df_01_aligned.index.droplevel(
        ["Original Protein IDs", "Compartment"]
    )
    ...
    for el in df_01_aligned.index.get_level_values("Protein IDs"):
        cs = [
            el
            for el in index_mapping.xs(el, level="Protein IDs", axis=0)
            .Compartment.iloc[0, :]
            .values
            if type(el) == str
        ]
...
def format_data_long(
    ...
    df_index.set_index(
        ["Original Protein IDs", "Gene names", "Samples"] + index_cols, inplace=True
    )
    ...
    df_index.set_index(
        pd.Index(
            [
                split_ids(el)
                for el in df_index.index.get_level_values("Original Protein IDs")
            ],
            name="Protein IDs",
        ),
        append=True,
...
def format_data_pivot(
    ...
    ## Rename columns
    df_index = df.rename(
        columns={original_protein_ids: "Original Protein IDs", genes: "Gene names"},
        errors="raise",
    )
    if genes == original_protein_ids:
        df_index.insert(0, "Original Protein IDs", df_index["Gene names"])
    ...
    df_index.set_index(
        ["Original Protein IDs", "Gene names"] + index_cols, inplace=True
    )
...
def filter_SILAC_countvar(
    ...
    df_filtered = (
        df_stack_filtered.unstack([el for el in column_levels if el != "Set"])
...
def filter_msms_count(
    ...
    if len(column_levels) > 1:
        df_profiles = df.stack([el for el in column_levels if el != "Set"]).unstack(
            "Fraction"
        )
    else:
    ...
    minms = (
        len(set(df_profiles.columns.get_level_values("Fraction"))) * average_MSMS_counts
    )
...
def filter_consecutive(
    ...
    if len(fraction_order) == 0:
        df_profiles.sort_index(
            level="Fraction", axis=1, key=natsort_index_keys, inplace=True
        )
    else:
...
def normalize_sum1(df, sets: dict = dict(abundance="LFQ intensity")):
    """ """
    ## Fetch column levels and assert presence of the required sets and fraction annotations
    column_levels = df.columns.names
    # All sets present
    ...
    df_01norm = df_profiles[[sets["abundance"]]].div(
        df_profiles[sets["abundance"]].sum(axis=1), axis=0
    )
...
def rscores_multi(df: pd.DataFrame, pairs: list = []):
    ...
    if pairs == []:
        if "Map" in df.columns.names:
            pairs = [
                el
                for el in combinations(np.unique(df.columns.get_level_values("Map")), 2)
            ]
    ...
    correlations = delta.apply(
        lambda x: pd.Series(
            correlation_row(
                reshapeRow(x, replicates="Pair", variables="Fraction"), method="cosine"
            )
        ),
        axis=1,
    )
...
def process_mc_errorbars(mc, n=20, sample_size=0.75):
    score_iterations = pd.DataFrame(
        columns=["F1 score", "Recall", "Precision", "Class size"],
        index=pd.MultiIndex.from_tuples([], names=["Compartment", "iteration"]),
    )
